...
这个 App 走轻量亲友版路线：先手动维护关键数据，避免引入复杂后台、数据库或付费 API。
...
文件：`src/features/matches/matchData.ts`
...
首页 Hero 的比赛场次标题：由 `src/features/matches/matchData.ts` 自动汇总。
